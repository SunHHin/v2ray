...
update_v2ray.sh() {
	if [[ $_test ]]; then
		...
		local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://ghproxy.com/https://raw.githubusercontent.com/233boy/v2ray/old/v2ray.sh" | grep '_version' -m1 | cut -d\" -f2)
	fi
